...
from constants import REDUCED_IMAGE_SIZE as REDUCED_IMAGE_SIZE

IMAGE_PIXELS = IMAGE_SIZE**2
REDUCED_IMAGE_PIXELS = REDUCED_IMAGE_SIZE**2
NUM_HIDDEN_LAYERS = 1
HIDDEN_SIZES = [10] # length should equal NUM_HIDDEN_LAYERS
WEIGHT_DECAY = 0.00002


def weights(name='mnist', reuse=None, quantize=False, num_bits=8, pca=0):
    with tf.variable_scope(name, reuse=reuse):
        image_pixels = IMAGE_PIXELS if pca == 0 else REDUCED_IMAGE_PIXELS
...
def load_weights(sess, dir, model_name='mnist', num_bits=8, pca=0):
    image_pixels = IMAGE_PIXELS if pca == 0 else REDUCED_IMAGE_PIXELS
