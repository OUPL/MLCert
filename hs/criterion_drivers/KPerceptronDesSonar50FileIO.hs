...
m = fromInt 116 --the number of samples
des = fromInt 10 --the number of support vectors
epochs = fromInt 5
...
print_generalization_err test (model, train) =
  let corrects dataset = 
      ...
      percent_correct_test
        = fromIntegral (sum $ corrects test) / fromIntegral (fromNat (fromInt 92))
  in putStrLn
...
main = defaultMain [
    env (setupEnv "../data/sonar50train.dat" "../data/sonar50test.dat") $ \ ~ (test, train) ->
    bgroup "kpdsonar50" [ bench "DescriptionKernelPerceptron" $ nfIO (kperceptrondeshelper n des epochs 11.0 train test) ]
    ]
