...
m = fromInt 77 --the number of samples
des = fromInt 6 --the number of support vectors
epochs = fromInt 5
...
print_generalization_err test (model, train) =
  let corrects dataset = 
      ...
      percent_correct_test
        = fromIntegral (sum $ corrects test) / fromIntegral (fromNat (fromInt 73))
  in putStrLn
...
main = defaultMain [
    ...
    bgroup "kpdiris50" [ bench "DescriptionKernelPerceptron" $ nfIO (kperceptrondeshelper n des epochs 7.0 train test) ]
    ]
