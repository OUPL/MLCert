...
f32_map2 :: Nat -> (Float32 -> Float32 -> Float32) -> Float32_arr ->
            Float32_arr -> Float32_arr
f32_map2 = (\_ f a1 a2 -> Prelude.map (\(x,y) -> f x y) (Prelude.zip a1 a2))
...
type Weights = Float32_arr

type Bias = Float32

type Params = (,) Weights Bias

predict :: Nat -> Params -> A -> B
predict n p a =
  case p of {
   (,) w b -> f32_gt (f32_add (f32_dot n w a) b) f32_0}

type Hypers = Float32
  -- singleton inductive, whose constructor was mkHypers
  
alpha :: Hypers -> Float32
alpha h =
  h

update0 :: Nat -> Hypers -> ((,) A B) -> Params -> Params
update0 n h example_label p =
  case example_label of {
   (,) example label ->
    case eqb (predict n p example) label of {
     Prelude.True -> p;
     Prelude.False ->
      case p of {
       (,) w b -> (,)
        (f32_map2 n (\x1 x2 ->
          f32_add x1
            (f32_mult (f32_mult (alpha h) (float32_of_bool label)) x2)) w
          example) (f32_add b (float32_of_bool label))}}}

learner :: Nat -> T A B Hypers Params
learner n =
  Mk (\_ -> predict n) (update0 n)

perceptron :: Nat -> Nat -> Hypers -> ((((,) Float32_arr Prelude.Bool) -> R)
              -> Cont a1 (([]) ((,) Float32_arr Prelude.Bool))) -> (((,)
              Float32_arr Prelude.Bool) -> R) -> ((,) Float32_arr Float32) ->
              Cont a1
              ((,) ((,) Float32_arr Float32)
              (([]) ((,) Float32_arr Prelude.Bool)))
perceptron n epochs hypers =
  extractible_main (learner n) hypers epochs list_Foldable (\t ->
    ret (unsafeCoerce contMonad) t)
